...
with open("./data/topo/link2ind.pkl", "rb") as f:
    link2ind = pickle.load(f)

attr = pd.read_csv('./data/20201012150828attr.txt', sep='\t',
                   names=['linkid', 'length', 'direction', 'pathclass', 'speedclass', 'LaneNum', 'speedlimit', 'level',
                          'width'])
...
topo = pd.read_csv('./data/topo/topo_map.csv', names=['linkid', 'links'])
...
def save2TFrecord(path, file):
    datekey = file.split(".")[0].split('_')[0]
    weekday = datetime.datetime.strptime(datekey, "%Y%m%d").weekday()
    workday = 0 if weekday in (5, 6) else 1
    writer = tf.python_io.TFRecordWriter("./data/data/%s.tfrecord" % datekey)
    print("save tfrecord of file %s !" % os.path.join(path, file))
...
train_files = sorted(os.listdir("./data/traffic"))[-13:]
for file in train_files:
    save2TFrecord("./data/traffic", file)
